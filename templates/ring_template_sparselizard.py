...
interpolated = VectorDouble()
isFound = VectorChar()
b.interpolate(wholedomain, np.array(evalPoints).reshape(len(evalPoints[0])*3, order='F'), interpolated, isFound)
...
interpolated = VectorDouble()
isFound = VectorChar()
h.interpolate(wholedomain, np.array(evalPoints).reshape(len(evalPoints[0])*3, order='F'), interpolated, isFound)
interp3 = np.array(interpolated).reshape(3,len(evalPoints[0]), order='F')
